...
WMouseAreaButton {
    ...
    function closeWindow() {
        Compositor.dispatch(`hl.dsp.window.close({window = "address:${root.hyprlandClient?.address}"})`)
    }
    ...
    onClicked: event => {
        if (event.button === Qt.LeftButton) {
            ...
            Compositor.dispatch(`hl.dsp.focus({window = "address:${root.hyprlandClient?.address}"})`)
            GlobalStates.overviewOpen = false;
    ...
    WMenu {
        ...
        Action {
            ...
            onTriggered: {
                Compositor.dispatch(`hl.dsp.window.pin({window = "address:${root.hyprlandClient?.address}"})`);
            }
        }
        Action {
            icon.name: "empty"
